    ...
    for x in range(pageNO if pageNO is not None else 1, int(val/rowperpage)+1 if pageNO is None else pageNO+1):
        try:
            ...
            for i in range (n):
                ...
                c='%0.7f'%data[i]['current_price'] if data[i]['current_price'] is not None else 0
...
def index():
	...
	return render_template('myWork.html',Cryptocurrencies=Cryptocurrencies_data,Markets=Markets_data,MarketCap=MarketCap_data
