    ...
    for x in lenght:
        ...
        try:
            ...
            for i in range(n):
                ...
                if float(dKL) == 1:
                    dKL = '1.00'
                a = int(data[i]['market_cap_rank']
                        ) if data[i]['market_cap_rank'] is not None else 0
                b = data[i]['name'] if data[i]['name'] is not None else 0
                f = data[i]['symbol'].upper(
                ) if data[i]['symbol'] is not None else 0
                ...
                d = int(data[i]['total_volume']
                        ) if data[i]['total_volume'] is not None else 0
                ...
                h1 = char_len(
                    data[i]['ath_date'], 10) if data[i]['ath_date'] is not None else 0
                k1 = data[i]['atl'] if dK[i]['atl'] is not None else 0
                m1 = char_len(
                    data[i]['atl_date'], 10) if data[i]['atl_date'] is not None else 0
...
def index():
    ...
    global send_list
    send_list = sorted(send_list, key=lambda d: d['rank'], reverse=False)
    global header_data
    response = []
    ...
    if 'data' in header_data:
        ...
        MarketCap_data = "%0.f" % (
            header_data['data']['total_market_cap']['usd'])
        Vol_data = "%0.f" % (header_data['data']['total_volume']['usd'])
        BTCDominance_data = "%0.1f" % (
            header_data['data']['market_cap_percentage']['btc'])
    else:
...
def startWorking(start, limit):
    ...
    coins = db.execute(
        "SELECT ID, MAX(TimeValue) FROM Coin GROUP BY ID LIMIT "+start+", "+limit).fetchall()
    ...
    for coin in coins:
        ...
        if len(toInsert) > 0:
            db.executemany(
                "INSERT INTO Coin(ID, TimeValue, PriceValue) VALUES(?, ?, ?)", toInsert)
        db.commit()
...
def reload_data(page):
    ...
    i = (int(page) - 1) * PER_PAGE
    List1 = output[i:i + PER_PAGE]
    return make_response({"output": List1, "page": page}, 200)
...
def api():
    ...
    db = sqlite3.connect("test.db")
    output = db.execute("SELECT ID, printf('%.7f',MAX(PriceValue)), printf('%.7f',MIN(PriceValue)) FROM Coin WHERE ID IN ('"+"','".join(
        coins)+"') AND (TimeValue BETWEEN ? AND ?) GROUP BY ID HAVING MIN(TimeValue) <= ?", (start, end, minTime,)).fetchall()
