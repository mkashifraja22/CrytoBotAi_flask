...
def ALL(val, pageNO=None):
    ...
    for x in range(pageNO if pageNO is not None else 1, int(val/rowperpage)+1 if pageNO is None else pageNO+1):
        try:
            ...
            for i in range(n):
                ...
                h1 = char_len(data[i]['ath_date'],
                              10) if data[i]['ath_date'] is not None else 0
                k1 = data[i]['atl'] if dK[i]['atl'] is not None else 0
                m1 = char_len(data[i]['atl_date'],
                              10) if data[i]['atl_date'] is not None else 0
...
def index():
    ...
    return render_template('myWork.html', Cryptocurrencies=Cryptocurrencies_data, Markets=Markets_data, MarketCap=MarketCap_data, Vol=Vol_data, BTCDominance=BTCDominance_data, data=send_list)
